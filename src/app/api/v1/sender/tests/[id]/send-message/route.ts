...
import { TestStatus, PaymentStatus, GiftStatus } from "@prisma/client";
...
export async function POST(
  ...
  try {
    ...
    if (!channel || channel !== "WHATSAPP") {
      return NextResponse.json({ error: "Invalid channel. Only WHATSAPP is supported." }, { status: 400 });
    }
    ...
    // 11. Generate the public receiver URL
    const origin = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
    const testLink = `${origin}/t/${test.testToken}`;

    // 12. Send WhatsApp message through configured provider (WasenderAPI / Brevo / Mock)
    ...
    // 13. Update reminderSentAt only after successful WhatsApp send
