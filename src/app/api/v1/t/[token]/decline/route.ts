...
export async function POST(
  ...
  try {
    ...
    // 2. Perform atomic refund transaction
    await prisma.$transaction(async (tx) => {
      // Update test status to DECLINED
        ...
        if (wallet) {
          ...
          await tx.walletTransaction.create({
            data: {
              ...
              reference: refundRef,
              description: `Escrow refund of ₦${test.giftAmount.toLocaleString()} because receiver declined this test link`,
    ...
    let smsMessage = "";
    if (shouldRefund) {
      smsMessage = `Your loyalty test to ${test.receiverName} was declined by the receiver. The locked reward of ₦${amountStr} has been refunded back to your wallet.`;
    } else {
      smsMessage = `Your loyalty test to ${test.receiverName} was declined by the receiver. The locked reward of ₦${amountStr} was not refunded based on platform settings.`;
    }
    ...
    return NextResponse.json({
      message: shouldRefund
        ? "Test declined successfully and sender refunded."
        : "Test declined successfully and processed according to platform settings."
    }, { status: 200 });

  } catch (error: any) {
    console.error("❌ Decline receiver test error:", error);
